...
if(isset($_GET['searchdata'])){
	$sd = $_GET['searchdata'];
$selectQuery = "SELECT * FROM topics where name like '%".$sd."%' ORDER by id desc limit $recordstart,$pagesize";
	}
else {
$selectQuery = "SELECT * FROM topics  ORDER by id desc limit $recordstart,$pagesize";
}

if(isset($_GET['searchdata'])){
	$sd = $_GET['searchdata'];
$totalrecord = "select count(*) from topics where name like '%".$sd."%'";
	}
else {
	$totalrecord = "select count(*) from topics ";
	}
...
$table = "<table class='table table-hover'> <caption>Topics Table</caption> <tr><th>Sl</th><th>Name</th><th>Image</th><th>Action</th></tr>";
$sl =($recordstart+1) ;
while($row = $selectQueryResult->fetch_array()){
	//echo $row['price']."<br>";
	$table .= "<tr><td>".$sl++."</td><td class='clspn'>".$row['name']."</td><td><img src='../assets/images/icons/".$row['icon'].".png' width='120px'/></td><td><a href='#form_category' class='editbtn' data-cid='".$row['cid']."' data-sid='".$row['scid']."' data-id='".$row['id']."'><i class='fas fa-edit'></i></a><a class='delbtn' data-id='".$row['id']."'><i class='fas fa-trash'></i></a></td></tr>";
	}
$table .= "</table>";
if($totalRows > 0 ){
