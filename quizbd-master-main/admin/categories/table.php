...
if (isset($_GET['searchdata'])) {
	$sd = $_GET['searchdata'];
	$selectQuery = "SELECT * FROM categories where name like '%" . $sd . "%' ORDER by id desc limit $recordstart,$pagesize";
} else {
	$selectQuery = "SELECT * FROM categories  ORDER by id desc limit $recordstart,$pagesize";
}

if (isset($_GET['searchdata'])) {
	$sd = $_GET['searchdata'];
	$totalrecord = "select count(*) from categories where name like '%" . $sd . "%'";
} else {
	$totalrecord = "select count(*) from categories ";
}
...
$table = "<table class='table table-hover'> <caption>Categories Table</caption> <tr><th>Sl</th><th>Name</th><th>Image</th><th>Action</th></tr>";
$sl = ($recordstart + 1);
while ($row = $selectQueryResult->fetch_array()) {
	//echo $row['price']."<br>";
	$table .= "<tr><td>" . $sl++ . "</td><td class='clscn'>" . $row['name'] . "</td><td><img src='../assets/images/icons/" . $row['icon'] . ".png' width='120px'/></td><td><a href='#form_category' class='editbtn' data-editid='" . $row['id'] . "'><i class='fas fa-edit'></i></a><a class='delbtn' data-cid='" . $row['id'] . "'><i class='fas fa-trash'></i></a></td></tr>";
}
$table .= "</table>";
if ($totalRows > 0) {
